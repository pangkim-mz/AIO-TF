...
| **connector-web (EASM+웹공급망)** | "URL 넣어 보안 점검" 신규 커넥터. **설계·결정 확정, 미착수** — 상세 To-Do는 위 [세션 메모, 2026-06-05](b). web_asset 리터럴 추가(schema+storage 각 1줄) + TLS·헤더·노출JS(→OSV 재사용)·SRI 4종. 착수 지점: `packages/connector-web`(신규) + `apps/cli/src/web.ts`. | 중간 |
...
**권장 순서**: ~~GitHub 연결~~ → ~~인증 DB·OIDC~~ → ~~큐~~ → ~~토큰 발급 API~~ → ~~OSV CVSS 파싱~~ → ~~큐 재시도·회수~~(완료) → **connector-web MVP(설계 확정, 미착수 — 다음 1순위)** → API/웹 연결 → 데드레터/별도 워커 프로세스 / CVSS v2·v4(선택).
