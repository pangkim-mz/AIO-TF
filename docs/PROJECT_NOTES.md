...
- **테스트**: 기능과 함께 작성. 순수 로직은 프레임워크 무관 함수로 빼서 단위 테스트(예: `lib/format`, `lib/services`).
  현재 **143 passed / 4 skipped**(Postgres 계약 4건은 `DATABASE_URL` 있을 때만).
...
| **connector-web API/웹 연결** | 커넥터 MVP(CLI `scan:web`)는 **완료**(#26). 남은 것: `apps/api/src/scans.ts` `runScanJob` 디스패처에 web 케이스 + `POST /v1/scans/web` + 랜딩 히어로 입력창 연결. 착수 지점: `apps/api/src/scans.ts`·`apps/web/app/page.tsx`. **다음 1순위.** | 중간 |
...
**권장 순서**: ~~GitHub 연결~~ → ~~인증 DB·OIDC~~ → ~~큐~~ → ~~토큰 발급 API~~ → ~~OSV CVSS 파싱~~ → ~~큐 재시도·회수~~ → ~~connector-web MVP~~(완료 #26) → **connector-web API/웹 연결(다음 1순위)** → 데드레터/별도 워커 프로세스 / CVSS v2·v4(선택).
...
- connector-web(EASM)은 **passive MVP**다. 점검은 브라우저 동급 HTTP/TLS 핸드셰이크만(능동 포트스캔·서브도메인 열거·시크릿 스캔 미지원, Phase 2 + 도메인 소유권 검증 시 도입). 노출 JS 핑거프린트는 CDN/파일명 패턴 3종(jsDelivr/unpkg npm 경로·cdnjs·파일명 임베드)만 인식 → 그 외 번들/난독화 스크립트는 버전 미특정으로 OSV 조회 제외. API `POST /v1/scans/web`·워커·랜딩 입력 연결은 미구현(다음 슬라이스).
