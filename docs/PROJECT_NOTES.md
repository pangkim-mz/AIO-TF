...
| **connector-web Phase 2 — 능동 점검 API/웹 연결(선택)** | 커넥터 능동 점검 **완료**(#32 — 소유권 게이트·서브도메인 열거·시크릿 스캔·탈취 후보, CLI `--active`/`--token`). 남은 것: API `POST /v1/scans/web`에 `active` 옵션 + 워커에 `resolveTxt`/`resolveHost` 주입 + 대시보드에서 소유권 토큰 발급·검증 UX. 착수 지점: `apps/api/src/scans.ts` `runWebScan` + `activeScanUrl`. | 중간 |
...
**권장 순서**: ~~GitHub 연결~~ → ~~인증 DB·OIDC~~ → ~~큐~~ → ~~토큰 발급 API~~ → ~~OSV CVSS 파싱~~ → ~~큐 재시도·회수~~ → ~~connector-web MVP~~(#26) → ~~connector-web API/웹 연결~~(#27) → ~~connector-web Phase 2 능동 점검~~(#32) → **능동 점검 API/웹 연결 / 데드레터·별도 워커 / CVSS v2·v4(모두 선택)**.
...
- connector-web(EASM) 능동 점검(#32, Phase 2)은 **CLI(`scan:web --active`)에만 연결**됐다. API `POST /v1/scans/web`·워커·대시보드에는 아직 미연결(다음 슬라이스). 능동 점검은 **도메인 소유권(DNS TXT) 검증 통과 시에만** 실행된다 — 미검증이면 passive만. 서브도메인 열거는 흔한 라벨 23종 브루트포스(전수 아님 — CT 로그·재귀 열거 미구현). 시크릿 스캔은 페이지 HTML 본문만 대상(링크된 JS 본문 미수집)이고 고유접두 6종 패턴만(범용 토큰·엔트로피 기반 미탐지). 탈취 후보는 CNAME→SaaS 지문 9종 휴리스틱이라 수동 확인 필요(실제 미점유 여부는 미확정). 노출 JS 핑거프린트는 CDN/파일명 패턴 3종만 인식.
