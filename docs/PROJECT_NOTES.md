...
| **connector-web Phase 2(선택)** | passive MVP·API 연결 완료. 남은 것: 능동 점검(서브도메인 열거·시크릿 스캔) + 도메인 소유권 검증(DNS TXT 토큰). 착수 지점: `packages/connector-web/src/probe.ts`. | 중간 |
...
**권장 순서**: ~~GitHub 연결~~ → ~~인증 DB·OIDC~~ → ~~큐~~ → ~~토큰 발급 API~~ → ~~OSV CVSS 파싱~~ → ~~큐 재시도·회수~~ → ~~connector-web MVP~~(#26) → ~~connector-web API/웹 연결~~(완료 #27) → **데드레터/별도 워커 프로세스 / CVSS v2·v4 / connector-web Phase 2(모두 선택)**.
...
- connector-web(EASM)은 **passive MVP**다. 점검은 브라우저 동급 HTTP/TLS 핸드셰이크만(능동 포트스캔·서브도메인 열거·시크릿 스캔 미지원, Phase 2 + 도메인 소유권 검증 시 도입). 노출 JS 핑거프린트는 CDN/파일명 패턴 3종(jsDelivr/unpkg npm 경로·cdnjs·파일명 임베드)만 인식 → 그 외 번들/난독화 스크립트는 버전 미특정으로 OSV 조회 제외. API `POST /v1/scans/web`·워커·랜딩 입력 연결은 완료(#27). 능동 점검(서브도메인 열거·시크릿 스캔)은 Phase 2 미구현.
