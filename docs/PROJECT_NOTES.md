...
**다음 세션 시작 체크리스트(B)**:
- `git fetch` 후 origin/main 기준 새 브랜치(squash 머지 발산 주의 — 메모리 참조).
- `feat/iac-command-guide`(`b521b1f`)가 아직 미푸시·미머지 → **푸시/PR/머지 먼저 처리**하거나 B 작업에 묶을지 결정.
- 스코프 재확인(최소 vs +PR게이트) → composite `action.yml` + 폴링 셸 → 대시보드 YAML 복사 UI.
