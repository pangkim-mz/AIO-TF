...
35. 능동 점검 결과 UI 보완 — `/scan` 결과가 능동 점검 시에도 총계만 보여주던 것을 분해·확인으로 보강. **#34는 백엔드(커넥터)만이라 결과는 기존 일반 테이블(자산 #31·발견 #29 펼침)로 자동 노출됐지만, 스캔 직후 폼 결과엔 능동 점검 전용 표현이 없었다.** API: `ScanSummary`에 `subdomainCount`·`takeoverCount`·`secretCount`(검증 수행 시에만, `runWebScan`이 `scanned`에서 태그/sourceFindingId 접두로 집계). 웹: 순수 `activeScanSummary(summary)`(검증 시 "능동 점검 수행됨 — 서브도메인 N · 탈취 후보 M · 노출 시크릿 L", 미검증/비능동이면 null) + `ScanResult`에 ✅ 분해 줄(검증)·⚠ 안내 줄(미검증). CSS `.notice-info`. 코어 0줄. 테스트 +2(activeScanSummary 검증/null) + API 분해 카운트 단언 보강 → **182 passed / 4 skipped**, web:build 통과.
36. `98063a6` IaC 명령어 안내 UI (PR #19 머지) — `/scan` 클라우드/인프라(IaC) 섹션에 `plan.json` 생성 절차 + 복사 버튼(`IacCommandGuide`). 세션 메모(2026-06-09 #4)의 `feat/iac-command-guide`를 push→PR→squash 머지. 코어 0줄.
37. GitHub Actions 연동(방향 B) — 최소 슬라이스 (PR #20) — 고객 레포 CI에 step 하나로 푸시/PR마다 Terraform plan을 떠서 OmniGuard로 자동 스캔(수동 붙여넣기 불필요). **재사용 composite action**(`examples/github-action/action.yml`): `hashicorp/setup-terraform` → `init`/`plan -out=tfplan`/`show -json tfplan > plan.json` → `jq`로 `{plan,stackName}` 본문 구성 → `POST /v1/scans/iac`(Bearer 토큰) → 202 `{jobId}` 받아 `GET /v1/jobs/:id`를 `succeeded`/`failed`까지 폴링(기본 180s, D9 비동기 큐 계약 그대로). inputs(`omniguard-url`·`omniguard-token`·`working-directory`·`stack-name`·`terraform-version`·`poll-timeout-seconds`)/outputs(`job-id`·`status`). **클라우드 credentials는 고객 CI 밖으로 안 나감**(plan.json만 전송) — 방향 B 채택 근거. 예시 워크플로(`example-workflow.yml`)·`README.md` 동봉. 대시보드: `/scan` IaC 섹션에 `GithubActionGuide`(`<details>` 토글 + 워크플로 YAML 복사) 추가, 복사 로직은 `CopyableCode` 공용 컴포넌트로 추출해 `IacCommandGuide`와 공유(중복 제거). CSS `.gh-guide`. **백엔드 코어 0줄·의존성 0**(기존 `POST /v1/scans/iac`+토큰 발급 API #5 재사용, GitHub App 불필요). UI는 순수 클립보드라 단위 테스트 없음 — **182 passed / 4 skipped**, typecheck·web:build 통과. **PR 코멘트/Critical 게이트는 다음 슬라이스**(GitHub 토큰 권한 추가 필요, §6 세션 메모 (3)).
38. `16ee444` IaC 연동 사용 매뉴얼 보강 (PR #21) — `examples/github-action/README.md`를 비전문가용 매뉴얼로 전면 보강(등장인물 3개·준비물 체크리스트·내 PC+Cloudflare 터널 호스팅·시크릿/변수 등록·문제해결 표·한계). 사내 서버 없는 환경 가정, 코드 0줄. 같은 날 **로컬 e2e로 action 단계 실증**(`connector-iac/test/fixtures/plan.json`→`POST /v1/scans/iac`→폴링 succeeded, 발견 4·스택 demo-infra 영향도 68 그래프 전파 상속). 진행보고서(`docs/aio-item-report-omniguard.html`, 미추적)에 **p.3**(IaC 자동 연동 실증 — 대시보드·발견 펼침 캡처) 추가.
39. `1f432b5` 대시보드 보안 점검 리포트 MD 다운로드 — 대시보드에 `/report` 페이지 + nav "리포트" 링크 추가. 현재 점검 결과(자산·발견·위험 점수)를 **마크다운 리포트로 버튼 한 번에 내려받는다**. `apps/web/lib/report.ts`(순수: 조회 데이터→MD 문자열, 요약·심각도별 집계·발견 상세[자산·CVSS·위험 점수·영향 전파]; 기존 `buildFindingDetails`·`summarize`·`ASSET_TYPE_LABEL` 재사용 — 중복 0) + `app/report/page.tsx`(SSR 조회+미리보기) + `app/report/download-button.tsx`(클라이언트 — Blob으로 `.md` 브라우저 다운로드, 파일명 `omniguard-report-YYYY-MM-DD.md`). **브라우저 보안상 임의 로컬 폴더 저장 불가**라 전용 페이지로 모으고 다운로드는 브라우저 기본 폴더로(사용자 선택). 새 API 0, 코어(`schema`/`scoring`/`graph`/`storage`) 0줄, 의존성 0. 단위 테스트 5건(`test/report.test.ts`) → **187 passed / 4 skipped**. 실연: megazone.digital 스캔 후 `/report`에서 gsap 발견 포함 MD 미리보기·다운로드 확인. 발표 데모(megazone.digital — 노출 JS gsap 2.0.1의 GHSA-6g8v-hpgw-h2v7/CVE-2020-28478 자동 연결, CVSS 7.5 HIGH, 위험 68) 중 나온 고도화 아이디어.
...
**[세션 메모, 2026-06-09 #4] IaC 명령어 안내 UI(완료) + GitHub Actions 연동(B) 설계 — 다음 세션 이어가기**

이 세션은 두 가지였다. (1) IaC 스캔 폼 안내 UI를 만들어 커밋했고, (2) GitHub 연동(방향 B)을
설계만 확정하고 코드는 **다음 세션으로 미뤘다**(사용자: "오늘은 여기까지").

**(1) IaC 명령어 안내 UI — 완료, 브랜치 `feat/iac-command-guide`(커밋 `b521b1f`, 미푸시·미PR).**
`/scan`의 클라우드/인프라(IaC) 섹션은 사용자가 직접 `plan.json`을 만들어 붙여넣어야 하는데
안내가 없었다. 폼 위에 생성 절차 + 복사 버튼 코드 블록을 추가:
- `apps/web/app/scan/forms.tsx`: `IacCommandGuide` 클라이언트 컴포넌트(신규). 안내 문구 +
  3줄 명령어(`terraform init` / `terraform plan -out=tfplan` / `terraform show -json tfplan > plan.json`)
  코드 블록 + 복사 버튼(`navigator.clipboard.writeText`, 복사 시 2초간 "복사됨 ✓", `COPY_RESET_MS` 상수).
- `apps/web/app/scan/page.tsx`: IaC 섹션 `<h2>` 아래·폼 위에 `<IacCommandGuide />` 배치.
- `apps/web/app/globals.css`: `.cmd-guide`/`.cmd-guide-intro`/`.code-block`/`.code-copy` 추가(기존 NordVPN 토큰 재사용).
- 코어 0줄. `pnpm web:build`·`pnpm typecheck` 통과. (이 UI는 순수 클립보드 동작뿐이라 단위 테스트 없음.)
- **주의**: 현재 IaC 폼은 **파일 업로드가 아니라 textarea 붙여넣기**다. 사용자 원안의 "파일을 업로드"
  문구는 실제 UX에 맞춰 "파일 내용을 아래 입력란에 붙여넣어 주세요"로 적었다. 진짜 파일 업로드(드래그&드롭/
  파일 선택)를 원하면 별도 슬라이스로 가능(사용자에게 옵션으로 안내함, 미결).

**(2) GitHub Actions 연동(방향 B) — 설계 확정, 코드 미착수. 다음 세션 1순위.**
사용자 질문 "GitHub 연동하면 이 수동 과정이 필요 없지 않나? 현실적으로 타 프로젝트 GitHub Action에
연결한다는 건가?"에 대한 결론과 설계:

- **현실 모델**: OmniGuard가 고객 레포에 들어가는 게 아니라, **고객(타 프로젝트) 레포의 CI 파이프라인에
  OmniGuard로 결과를 보내는 step 하나를 추가**한다. 푸시/PR마다 자동 스캔 → 수동 붙여넣기 불필요.
  **기존 수동 UI(textarea)는 온보딩·일회성용으로 병행 유지**(사용자 명시 요구).
- **왜 B인가**(A=소스 .tf 직접분석, C=우리가 plan 실행 과 비교):
  - **A(소스 직접)**: 레포만 연결하면 되나, plan.json이 아니라 정적 .tf 파싱이라 변수·모듈·remote state
    미해석으로 부정확 + `connector-iac`(plan JSON 전용)에 새 파서 필요. → 정확도 손해, "빠른 미리보기"용만.
  - **B(고객 CI에서 plan→API 업로드)**: plan.json 그대로라 **수동과 정확도 동일**, 자동화됨.
    **클라우드 credentials가 고객 CI 밖으로 안 나감**(SaaS 신뢰 모델 적합). 기존 `POST /v1/scans/iac` +
    토큰 발급 API(#5) 재사용 → **백엔드 코어 0줄**. → 채택.
  - **C(GitHub App + 우리가 plan 실행)**: 완전 자동이나 **클라우드 credentials를 우리가 받아야** 함 → 보안
    부담 커서 SaaS로는 비권장.
- **B 동작 흐름**:
  ```
  [고객 레포 my-infra]  .github/workflows/omniguard.yml
    terraform init → plan -out=tfplan → show -json tfplan > plan.json
    → curl POST {OMNIGUARD_URL}/v1/scans/iac  (Authorization: Bearer ${{ secrets.OMNIGUARD_TOKEN }})
                                                  → 큐 → connector-iac → 점수 → 영속 → 대시보드 반영
  ```
- **만들 산출물(최소 슬라이스부터)**:
  1. **재사용 composite Action**(`action.yml`): init→plan→show→`POST /v1/scans/iac`. 고객은 자기
     워크플로에 `uses:` 한 줄. 위치 후보: `examples/github-action/` 또는 별도 작은 공개 레포.
     (스캔이 비동기라 202+jobId 받은 뒤 `GET /v1/jobs/:id`를 완료까지 폴링하는 셸 로직 포함 — D9.)
  2. **대시보드 안내 + YAML 복사 버튼**: `/scan` IaC 섹션에 "GitHub로 자동 연동" 토글/탭 +
     워크플로 YAML을 **이번에 만든 `code-block`/`code-copy` 컴포넌트 그대로 재사용**해 복사 제공.
     토큰은 발급 API(#5)로 만든 값을 안내(레포 시크릿 `OMNIGUARD_TOKEN`에 넣으라고).
  3. **(선택, 다음 단계) PR 코멘트/게이트**: Action이 결과를 PR에 코멘트 + Critical 발견 시 CI fail.
     → 이건 **GitHub 토큰 권한 추가 필요**. "plan 떠서 결과 보내기"만이면 **GitHub App 없이 API 토큰
     하나로 충분**(권한 경계 주의점).
- **사용자에게 받은 스코프 결정**: 미정. AskUserQuestion에서 "최소(Action+안내) / 최소+PR코멘트·게이트 /
  설계만"을 물었으나 사용자가 "다음부터 이어서 하도록 기록, 오늘은 여기까지"로 답함. → **다음 세션 시작 시
  스코프(최소 vs +PR게이트)부터 재확인**하고 착수. 권장: **최소 슬라이스(Action 템플릿 + 대시보드 YAML 복사)**
  먼저(백엔드 0줄·API 토큰만·GitHub App 불필요). 이번 IaC UI의 code-block 컴포넌트를 재사용하면 빠르다.

**다음 세션 시작 체크리스트(B)** — **완료(#36·#37, 2026-06-10)**:
- ✅ `feat/iac-command-guide`(`b521b1f`) push→PR #19→squash 머지(`98063a6`).
- ✅ 스코프 = **최소 슬라이스**(사용자 확정) → composite `examples/github-action/action.yml`(폴링 셸 포함) + 대시보드 `/scan` `GithubActionGuide`(YAML 복사). 백엔드 0줄·의존성 0.
- 다음 슬라이스(선택): **PR 코멘트/Critical 게이트** — Action이 결과를 PR에 코멘트 + Critical 시 CI fail. GitHub 토큰 권한 추가 필요(위 세션 메모 (2)-3).

**[세션 메모, 2026-06-10] GitHub Actions IaC 연동(B) 최소 + 사용 매뉴얼 + 로컬 e2e 실증**

시작 HEAD=origin/main=`4048d8d`(+미머지 `feat/iac-command-guide` `514caf3`), 182 passed. 사용자 스코프 답=**최소 슬라이스** + **먼저 머지 후 B 착수**.

1. **선행 머지**: `feat/iac-command-guide` push→PR #19→squash(`98063a6`).
2. **B 최소 슬라이스(#37)**: PR #20→`f9da4ae`. `examples/github-action/`(action.yml 재사용 composite: setup-terraform→init/plan/show→jq 본문→`POST /v1/scans/iac`→202 jobId→`GET /v1/jobs/:id` 폴링; example-workflow.yml; README.md) + 대시보드 `/scan` `GithubActionGuide`(`<details>`+워크플로 YAML 복사, 복사 로직 `CopyableCode` 공용화). 백엔드 0줄·의존성 0·GitHub App 불필요.
   - **함정 재발**: B 커밋을 또 bash에서 `git commit -m @'...'@`(PowerShell here-string)로 써서 제목이 `@`→squash가 main에 `@ (#20)`로 들어감. `--amend`로 교정 후 `git push --force-with-lease origin main`(사용자 승인)→`f9da4ae`. **bash 툴 커밋 메시지는 `-F 파일` 또는 heredoc, `@'...'@`는 PowerShell 전용.**
3. **사용 매뉴얼 보강**: PR #21→`16ee444`. `examples/github-action/README.md`를 비전문가용 매뉴얼로 전면 보강 — 등장인물 3개·준비물 체크리스트·**내 PC + Cloudflare 터널** 호스팅(Windows PS: 강한 토큰→`OMNIGUARD_TOKENS`(JSON)→`cloudflared --url`→`/health`)·대상 레포 시크릿/변수·문제해결 표·한계. (소스 확인: `OMNIGUARD_TOKENS`=JSON `{"<token>":{"tenantId","role"}}`, 기본 `HOST=0.0.0.0:3000`.)
4. **로컬 end-to-end 실증**: action.yml 단계를 로컬 재현 — `connector-iac/test/fixtures/plan.json`(S3 public·SG 0.0.0.0/0·RDS 퍼블릭+미암호화)을 `POST /v1/scans/iac`→폴링 succeeded(자산5·발견4·점수68). 발견 4건(공개 S3 ACL·전체개방 인바운드·퍼블릭 액세스 HIGH·미암호화 MEDIUM) + **스택 demo-infra(own 0)가 그래프 전파로 영향도 68 상속** 확인. 대시보드(:3001) 캡처. terraform/jq 미설치 환경이라 픽스처+node로 대체(터미널 호출만 생략, 파이프라인 전부 실연). **데모 서버는 세션 끝에 taskkill로 정리(포트 3000/3001 점유 child 직접 kill — 메모리 함정).**
5. **진행보고서 갱신**(`docs/aio-item-report-omniguard.html`, 미추적): 진행상황·Next Step에 GitHub Actions 자동 연동 반영(좌측 패널 오버플로는 여백 미세축소로 해소) + **p.3 신규**(참고자료 — IaC 자동 연동 실증: 대시보드·발견 펼침 캡처, p.1·2 푸터 `/2`→`/3`).

**다음 후보(모두 선택)**: GitHub Action PR 코멘트·Critical 게이트 / connector-web 시크릿 JS 본문 수집 / 큐 DLQ·별도 워커 / CVSS v2·v4. **운영 배포(상시 URL+Postgres)** 가 실전 활용의 선결 과제(현재 로컬/터널은 데모용).
...
| ~~GitHub Actions 연동(방향 B) 최소~~ | **완료**(#37). 재사용 composite `examples/github-action/action.yml`(init→plan→show→`POST /v1/scans/iac`+jobs 폴링) + 대시보드 `/scan` `GithubActionGuide`(워크플로 YAML 복사). 백엔드 0줄·의존성 0. 남은 것: PR 코멘트/Critical 게이트(선택). | — |
...
**권장 순서**: ~~GitHub 연결~~ → ~~인증 DB·OIDC~~ → ~~큐~~ → ~~토큰 발급 API~~ → ~~OSV CVSS 파싱~~ → ~~큐 재시도·회수~~ → ~~connector-web MVP~~(#26) → ~~connector-web API/웹 연결~~(#27) → ~~Phase 2 능동 점검~~(#32) → ~~Phase 2 API/웹 연결~~(#33) → ~~능동 점검 고도화: CT 로그·takeover 확증~~(#34) → ~~IaC 명령어 안내 UI~~(#36) → ~~GitHub Actions 연동(B) 최소~~(#37) → **시크릿 JS 본문 수집 / GitHub Action PR 게이트 / 데드레터·별도 워커 / CVSS v2·v4(모두 선택)**.
