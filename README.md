...
- 페이지: 랜딩(히어로 URL 보안 점검 입력창), 대시보드(요약+상위 발견/영향도), **서비스(교차 도메인 통합 리스크)**,
  자산, 발견(심각도순), 영향도(전파/근원), 스캔(서버 액션으로 화면에서 직접 npm/벤더/IaC/서비스/웹 스캔 실행 → 성공 시 조회 페이지 재검증).
  스캔은 API에서 비동기지만, `ApiClient`가 작업 완료까지 서버측에서 폴링해 기존 동기형 UI를 그대로 유지한다.
- **발견·영향도 테이블은 행을 클릭하면 펼쳐진다**: 발견=설명·위험 점수 분해(가중치 factors)·대상 자산·영향 전파,
  영향도=자체↔전파 비교·상속 근원·전파 경로·직접 발견. 새 API 없이 조회 조합으로 만든다
  (`lib/findings.ts`·`lib/impact.ts` 순수 함수 + 단위 테스트, `findings-table.tsx`·`impact-table.tsx` 클라이언트).
